...
function EmptyState({ sites, onSelect }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center space-y-6 p-6 text-center">
      <div className="flex h-20 w-20 items-center justify-center rounded-full border border-slate-700/80 bg-slate-800 text-3xl shadow-xl shadow-slate-950">
        &#127963;
      </div>
      ...
      <div className="max-w-md space-y-2">
        ...
        <p className="text-xs leading-relaxed text-slate-400">
          Select any map pin across Tunisia or choose a featured scan below to inspect
          photogrammetry artifacts in high-definition 3D.
        </p>
      </div>
...
export default function Inspector({ site, sites, guideEnabled, onClose, onSelect }) {
  const [tab, setTab] = useState('context');

  // A different site starts on its own overview rather than mid-conversation.
  useEffect(() => setTab('context'), [site?.id]);
  ...
  return (
    <div className="flex flex-col space-y-5 p-4 lg:p-6">
      ...
      {/* The scan stays visible while the panel below switches between tabs. */}
